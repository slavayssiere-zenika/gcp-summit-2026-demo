...
def extract_metadata_from_session(session):
    """
    Exhaustively parses history events to extract the latest steps, thoughts, and data.
    Ensures consistency between live queries and history endpoints, even after Redis reload.
    """
    steps = []
    seen_steps = set()
    thoughts = []
    ...
    # Session might be an object or a dictionary (if direct from Redis)
    ...
    for event in events:
        ...
        # 1. Capture Thoughts (Gemini 2.0 support)
        for part in parts:
            ...
            if thought_val:
                ...
                if text: thoughts.append(text)

        # 2. Capture Tool Calls (Steps)
        for part in parts:
            # Check for multiple possible field names (SDK variations)
            tcall = safe_get(part, 'tool_call') or safe_get(part, 'function_call') or safe_get(part, 'call')
        ...
        # Alternative tool call structures in some ADK versions
