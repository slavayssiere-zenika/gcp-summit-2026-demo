...
async def login(email: str, password: str) -> str:
    ...
    async with httpx.AsyncClient(verify=VERIFY_SSL) as client:
        try:
            resp = await client.post(url, json={"email": email, "password": password}, timeout=10.0)
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if resp.status_code == 429:
                        print(
                            f"  Rate limited (429) during fetch. Retrying in {backoff}s... (Attempt {attempt+1}/{max_retries})")
...
async def update_competency(token: str, competency_id: int, update_data: Dict[str, Any]):
    ...
    async with httpx.AsyncClient(verify=VERIFY_SSL) as client:
        for attempt in range(max_retries):
            try:
                resp = await client.put(url, headers=headers, json=update_data, timeout=10.0)
    ...
    for comp, fixed_data in corrupted_found:
        ...
        if result is True:
            ...
            if existing_id:
                ...
                print(
                    f"  Warning: Could not find a clean existing competency with name '{update_payload['name']}' to merge into.")
