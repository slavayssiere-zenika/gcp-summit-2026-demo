...
# Email du Service Account cv_api autorisé à appeler /bulk_tree via OIDC (batch taxonomy).
# Injecté via Terraform (cr_competencies.tf → BATCH_CALLER_SA_EMAIL = google_service_account.cv_sa.email).
...
def _decode_and_validate(token: str) -> dict:
    ...
    try:
        ...
        if unverified_header.get("alg") == "RS256":
            # Potentiel token OIDC Google (sa-cv via Cloud Scheduler ou batch pipeline)
            try:
                ...
                if BATCH_CALLER_SA_EMAIL and caller_email == BATCH_CALLER_SA_EMAIL:
                    ...
                    # (jwt_payload.get("role")) — le payload OIDC brut ne contient pas ce claim.
