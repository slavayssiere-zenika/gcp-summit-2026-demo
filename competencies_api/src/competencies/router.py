...
Architecture modulaire (< 400 lignes par fichier) :
  ...
  - analytics_router.py    : bulk-scoring-all, coverage, analytics (agency, skill-gaps, similar)
  - helpers.py             : fonctions utilitaires partagées (scoring v2, sérialisation...)
  - ai_scoring.py          : moteur Gemini v2 (_compute_ai_score, _score_all_bg, _bulk_scoring_all_bg)
...
ORDRE CRITIQUE (règle FastAPI — routes statiques AVANT wildcards) :
  ...
  3. analytics_router     : /bulk-scoring-all/..., /stats/coverage, /analytics/...
  4. assignments_router   : /user/{user_id}/...                         →  dernière (wildcards)
  5. public_router        : /pubsub/user-events  (sans auth)
"""
...
from src.competencies.analytics_router import scheduler_router as analytics_scheduler_router
