...
from src.competencies.schemas import (
    AgencyCompetencyCoverage,
    AgencyCompetencyItem,
    SimilarConsultant,
    SimilarConsultantsResult,
    SkillGapItem,
    SkillGapResult,
)
...
async def get_scoring_stats(
    ...
    users_ok = (
        await db.execute(
            select(func.count())
            .select_from(scored_per_user_sub)
            .where(scored_per_user_sub.c.scored_count >= min_scored_count)
        )
    ).scalar_one()
    avg_scored_row = (
        await db.execute(select(func.avg(scored_per_user_sub.c.scored_count)))
    ).scalar_one()
    ...
    return {
        "total_users_with_competencies": total_with_comps,
        "users_with_min_scored": users_ok,
        "users_without_sufficient_score": total_with_comps - users_ok,
        "coverage_pct": coverage_pct,
        "avg_scored_per_user": avg_scored,
...
async def get_agency_competency_coverage(
                ...
                for u in users:
                    ...
                    agency = next(
                        (
                            ...
                            and tag.get("category") in ("agence", "agency", "Agence")
                        ),
                        None,
                    )
                    if uid and agency:
    ...
    stmt = (
        ...
        .where(
            ~Competency.id.in_(
                ...
                .where(Competency.parent_id.isnot(None))
                .distinct()
            )
        )
    )
    ...
    items = [
        AgencyCompetencyItem(
            ...
            count=vals["count"],
            avg_ai_score=(
                round(sum(vals["scores"]) / len(vals["scores"]), 2)
                if vals["scores"]
                else None
            ),
        )
...
async def get_skill_gaps(
    ...
    competency_ids: Optional[List[int]] = Query(None),
    ...
    count_rows = {
        ...
        for r in (
            await db.execute(
                ...
                .where(
                    user_competency.c.competency_id.in_([c.id for c in target_comps])
                )
    ...
    gaps = sorted(
        [
            SkillGapItem(
                ...
                consultants_with_skill=count_rows.get(c.id, 0),
                consultants_in_pool=pool_size,
                coverage_pct=min(
                    100.0, round(count_rows.get(c.id, 0) / max(1, pool_size) * 100, 1)
                ),
            )
            for c in target_comps
            if count_rows.get(c.id, 0) / pool_size <= min_coverage
        ],
...
@router.get(
    "/analytics/similar-consultants/{user_id}", response_model=SimilarConsultantsResult
)
async def get_similar_consultants(
    user_id: int, top_n: int = Query(5, ge=1, le=20), db: AsyncSession = Depends(get_db)
):
    ...
    results = sorted(
        [
            SimilarConsultant(
                user_id=uid,
                common_competencies=len(ref_set & other_set),
                jaccard_score=round(
                    len(ref_set & other_set) / len(ref_set | other_set), 3
                ),
                shared_competency_names=sorted(
                    [
                        ref_names.get(cid, user_comp_names[uid].get(cid, ""))
                        for cid in ref_set & other_set
                    ]
                ),
            )
            for uid, other_set in user_comp_sets.items()
        ],
        key=lambda x: -x.jaccard_score,
    )[:top_n]
...
async def get_taxonomy_quality(db: AsyncSession = Depends(get_db)):
    ...
    return compute_taxonomy_quality(all_comps)
