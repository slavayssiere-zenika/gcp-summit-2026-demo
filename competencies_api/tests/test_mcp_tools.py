...
def test_mcp_app_call_success(mocker):
    ...
    resp = client.post("/mcp/call", json={"name": "list_competencies",
                       "arguments": {}}, headers={"Authorization": "Bearer x"})
    assert resp.status_code == 200
...
async def test_mcp_server_main(mocker):
    ...
    # Patch la référence importée dans mcp_server (pas le module source)
