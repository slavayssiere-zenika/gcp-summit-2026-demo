...
async def test_mcp_server_call_tool_propagates_headers():
    ...
    with patch("mcp_server.httpx.AsyncClient") as mock_client_class:
        ...
        # On intercepte handle_list_competencies pour ne pas tester la logique métier,
        # mais juste garantir que le client a été instancié avec les bons headers.
        with patch("mcp_server.handle_list_competencies", new_callable=AsyncMock) as mock_handle:
            mock_handle.return_value = []

            tracer = trace.get_tracer(__name__)
            with tracer.start_as_current_span("test_mcp_tool_span"):
                await call_tool("list_competencies", {"skip": 0, "limit": 10})
