        ...
        if hasattr(route, "dependencies"):
            for dep in route.dependencies:
                if isinstance(dep.dependency, VerifyJwtOrOidc):
                    overrides[dep.dependency] = lambda: {"sub": "1", "allowed_category_ids": [1]}
        if hasattr(route, "dependant"):
            for dep in route.dependant.dependencies:
                if isinstance(dep.call, VerifyJwtOrOidc):
                    overrides[dep.call] = lambda: {"sub": "1", "allowed_category_ids": [1]}
...
def test_bulk_import_tree_authorized(client):
    ...
    for route in app.routes:
        if hasattr(route, "dependencies"):
            for dep in route.dependencies:
                if isinstance(dep.dependency, VerifyJwtOrOidc):
                    overrides[dep.dependency] = lambda: {"sub": "1", "role": "admin", "allowed_category_ids": []}
        if hasattr(route, "dependant"):
            for dep in route.dependant.dependencies:
                if isinstance(dep.call, VerifyJwtOrOidc):
                    overrides[dep.call] = lambda: {"sub": "1", "role": "admin", "allowed_category_ids": []}
    with patch.dict(app.dependency_overrides, overrides):
        payload = {
            "tree": {
                "Language": {
