...
def patch_cache(mock_redis):
    with patch("src.services.tree_resolution.get_cache", new=mock_redis.get), \
            patch("src.services.tree_resolution.set_cache", new=mock_redis.set), \
            patch("src.services.tree_resolution.delete_cache", new=mock_redis.delete):
        yield


@pytest.fixture
def resolver(mock_db, mock_drive_api):
    return TreeResolver(mock_db, mock_drive_api)


@pytest.mark.asyncio
async def test_get_cached_roots_hit(resolver, mock_redis):
    mock_redis.get.return_value = '[{"id": 1}]'
    roots = await resolver._get_cached_roots()
    assert roots == [{"id": 1}]


@pytest.mark.asyncio
async def test_get_cached_roots_corrupt(resolver, mock_redis):
    mock_redis.get.return_value = 'corrupt'
    roots = await resolver._get_cached_roots()
...
@pytest.mark.asyncio
async def test_set_cached_roots(resolver, mock_redis):
    await resolver._set_cached_roots([{"id": 1}])
    mock_redis.set.assert_called_once()


@pytest.mark.asyncio
async def test_invalidate_roots_cache(resolver, mock_redis):
    await resolver.invalidate_roots_cache()
    mock_redis.delete.assert_called_once()
...
async def test_resolve_root_and_parent_found_in_cache(resolver, mock_redis):
    mock_redis.get.side_effect = lambda k: "folder1" if "drive:name" in k else None
...
@pytest.mark.asyncio
async def test_set_cached_roots_exception(resolver, mock_redis):
...
async def test_resolve_root_and_parent_cache_hit_missing_root(resolver, mock_redis, mock_drive_api):
    def redis_get(k):
        if "drive:name" in k:
            return "parent"
        if "drive:graph:f1" == k:
            return "2"  # root 2 not in roots list
        return None
    mock_redis.get.side_effect = redis_get
