...
@patch('src.services.ingestion_kpi_service.get_redis')
async def test_get_ingestion_stats(mock_get_redis, mock_compute, service, mock_db):
...
@pytest.mark.asyncio
@patch('src.services.ingestion_kpi_service.get_redis')
async def test_quality_gate_batch(mock_get_redis, service, mock_db):
    mock_redis = MagicMock()
