...
@pytest.mark.asyncio
async def test_discover_delta_bottom_up_trashed_file_is_out_of_scope(mock_db, mock_google_client, mock_pubsub):
    """
    ...
    with patch("src.services.tree_resolution.TreeResolver.load_roots", return_value=[{"id": 1, "google_folder_id": "root_id_1", "tag": "tag1", "folder_name": "Root", "excluded_folders": None}]), \
        ...
        fake_file = {"id": "trashed_file_id", "name": "CV Trashed", "mimeType": "application/vnd.google-apps.document",
                     "trashed": True, "parents": ["parent1"], "modifiedTime": "2026-05-05T12:00:00Z"}
...
@pytest.mark.asyncio
async def test_ingest_batch_publishes_delete_action_for_out_of_scope(mock_db, mock_google_client, mock_pubsub):
    """
