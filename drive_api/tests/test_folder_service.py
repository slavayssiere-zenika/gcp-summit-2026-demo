...
@pytest.mark.asyncio
@patch('src.services.folder_service.get_redis')
@patch('src.services.folder_service.get_drive_service')
async def test_add_folder_success(mock_get_drive, mock_get_redis, service, mock_db):
...
@pytest.mark.asyncio
@patch('src.services.folder_service.get_redis')
async def test_update_folder_success(mock_get_redis, service, mock_db):
    mock_redis = MagicMock()
    mock_get_redis.return_value = mock_redis
...
@pytest.mark.asyncio
@patch('src.services.folder_service.get_redis')
async def test_delete_folder(mock_get_redis, service, mock_db):
    mock_redis = MagicMock()
    mock_get_redis.return_value = mock_redis
...
@pytest.mark.asyncio
@patch('src.services.folder_service.get_redis')
async def test_invalidate_drive_cache(mock_get_redis):
    mock_redis = MagicMock()
    mock_redis.scan_iter.side_effect = [["key1"], ["key2"], []]
    mock_get_redis.return_value = mock_redis
    
    res = FolderService.invalidate_drive_cache()
    assert res == 2
