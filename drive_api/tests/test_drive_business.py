...
def test_folders_post_requires_jwt():
...
def test_folders_get_requires_jwt():
...
def test_folders_delete_requires_jwt():
...
def test_add_folder_extracts_id_from_url():
...
def test_status_has_all_required_fields():
...
def test_sync_returns_started_when_drive_accessible(mocker):
...
def test_sync_returns_403_when_drive_permission_lost(mocker):
...
def test_retry_errors_no_errors_to_retry():
