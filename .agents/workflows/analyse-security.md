...
- **Propagation JWT Obligatoire** : Vérifier que chaque `mcp_server.py` propage le JWT entrant via `auth_header_var` (ContextVar) dans les headers des appels HTTP sortants vers les APIs data. L'absence de propagation implique que les appels MCP arrivent sans JWT → rejet 401 ou, pire, bypass si le backend n'a pas de guard.
