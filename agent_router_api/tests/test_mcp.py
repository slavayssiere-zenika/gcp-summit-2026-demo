...
import httpx
import pytest
from mcp_client import (MCPHttpClient, MCPSseClient, auth_header_var,
                        get_competencies_mcp, get_cv_mcp, get_items_mcp,
                        get_loki_mcp, get_users_mcp)
...
async def test_mcp_http_client_list_tools(mock_httpx):
    ...
    mock_httpx.get.assert_called_with("http://fake.url/mcp/tools", timeout=30.0)
...
async def test_mcp_sse_client_list_tools(mocker):
    _mock_sse = mocker.patch("mcp.client.sse.sse_client", new_callable=MagicMock)  # noqa: F841
    mock_session = mocker.patch("mcp.client.session.ClientSession", new_callable=MagicMock)
    ...
    mock_stack_enter.side_effect = [("stream1", "stream2"), mock_session_inst]
