                ...
                async def _log_cache_hit_bq():
                    try:
                        ...
                        async with httpx.AsyncClient(timeout=10.0) as bq_client:
                            ...
                            }, headers=headers_bq, timeout=10.0)
                    except Exception:
                        raise
        ...
        if current_assistant_msg:
            for part in parts:
                ...
                if fres:
                    ...
                    try:
                        res_to_process = json.loads(raw_text)
                    except Exception as e:
                        logging.warning(f"Parse error: {e}")

                if res_to_process is not None:
                    if hasattr(res_to_process, 'model_dump'):
                        res_to_process = res_to_process.model_dump()
                    ...
                    if isinstance(res_to_process, dict) and "response" in res_to_process:
                        ...
                        if "usage" not in current_assistant_msg:
                            current_assistant_msg["usage"] = {"total_input_tokens": 0,
                                                              "total_output_tokens": 0, "estimated_cost_usd": 0}
            ...
            if u:
                ...
                current_assistant_msg["usage"]["total_input_tokens"] = max(
                    current_assistant_msg["usage"]["total_input_tokens"], it)
                current_assistant_msg["usage"]["total_output_tokens"] = max(
                    current_assistant_msg["usage"]["total_output_tokens"], ot)
...
async def _save_sessions(user_id: str, sessions: list):
...
async def _migrate_legacy_session(user_id: str, sessions: list) -> list:
    """
...
async def list_sessions(auth: HTTPAuthorizationCredentials = Depends(security)):
    ...
    sessions = await _load_sessions(jwt_user_id)

    if not sessions:
        sessions = await _migrate_legacy_session(jwt_user_id, sessions)
        if not sessions:
        ...
        await _save_sessions(jwt_user_id, sessions)
