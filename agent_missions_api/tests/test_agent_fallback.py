...
async def test_run_agent_query_redis_restore_error():
    ...
    mock_session_service.r = MagicMock()
            ...
            with patch("agent.app_logger.warning") as mock_logger:
                with patch("agent.create_agent", new=AsyncMock()):
                    with patch("agent.Runner", return_value=MagicMock()):
                        with patch(
                            "agent.run_agent_and_collect",
                            new=AsyncMock(return_value=("resp", [], [], 0, 0, None, "text_only")),
                        ):
                            with patch("agent.extract_metadata_from_session", return_value={}):
                                with patch("agent.log_tokens_to_bq"):
                                    ...
                                    msg = "[MISSIONS] Impossible de lire le contexte mission Redis: %s"
                                    mock_logger.assert_any_call(
                                        msg,
                                        "Redis down" if False else mock_logger.call_args[0][1],
                                    )
        ...
        with patch("agent.get_missions_context", return_value=None):
            with patch("agent.create_agent", new=AsyncMock()):
                with patch("agent.Runner", return_value=MagicMock()):
                    with patch(
                        "agent.run_agent_and_collect",
                        new=AsyncMock(return_value=("resp", [], [], 0, 0, mock_last_data, "text_only")),
                    ):
                            ...
                            with patch("agent.store_missions_context", side_effect=Exception("Redis write down")):
                                with patch("agent.app_logger.warning") as mock_logger:
                                    with patch("agent.log_tokens_to_bq"):
                                        ...
                                        msg = "[MISSIONS] Impossible de persister le contexte mission: %s"
                                        mock_logger.assert_any_call(
                                            msg,
                                            "Redis write down" if False else mock_logger.call_args[0][1],
                                        )
