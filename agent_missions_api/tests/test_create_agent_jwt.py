...
os.environ.setdefault("GEMINI_MISSIONS_MODEL", "gemini-stub")
...
def _reset_tools_cache():
    ...
    import agent as ag
    ag._MISSIONS_TOOLS_CACHE.clear()
    yield
    ag._MISSIONS_TOOLS_CACHE.clear()
...
async def test_create_agent_propagates_jwt_to_prompts_api(mocker):
    ...
    assert result_agent.instruction == "Instruction depuis prompts_api"
...
async def test_create_agent_fallback_on_prompts_api_401(mocker):
    ...
    assert "Tu es l'Agent Missions" in result_agent.instruction
