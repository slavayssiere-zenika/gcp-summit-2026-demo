...
# Redis DB 12 — distinct de tous les autres services :
#   HR=10, Ops=11, Router=9, items_api=1, missions_api_rest=8
REDIS_URL_DEFAULT = "redis://redis:6379/12"
...
class RedisSessionService(InMemorySessionService):
    ...
    def _save_all(self, session_id: str):
        ...
        try:
            ...
            self.r.set(f"adk:missions:sessions:{session_id}", dump, ex=self.ttl)
        except Exception as e:
            logger.error("Redis missions session save fail: %s", e)

    def _load_all(self, session_id: str):
        if not session_id:
            return
        try:
            raw = self.r.get(f"adk:missions:sessions:{session_id}")
            if raw:
                d = pickle.loads(raw)
            ...
            logger.error("Redis missions session load fail: %s", e)
