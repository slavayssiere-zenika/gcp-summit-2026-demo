resource "google_cloud_run_v2_service" "cv_api" {
  name     = "cv-api-${terraform.workspace}"
  location = var.region
  ingress  = "INGRESS_TRAFFIC_INTERNAL_LOAD_BALANCER"

  template {
    service_account = google_service_account.cv_sa.email
    ...
    containers {
      ...
      env {
        name  = "DATABASE_URL"
        value = "postgresql://${replace(google_service_account.cv_sa.email, ".gserviceaccount.com", "")}@${google_alloydb_instance.primary.ip_address}:5432/cv"
      }
      env {
        name  = "ROOT_PATH"
        value = "/api/cv"
...
resource "google_secret_manager_secret_iam_member" "cv_jwt_access" {
...
resource "google_alloydb_user" "cv_db_user" {
  ...
  user_id    = replace(google_service_account.cv_sa.email, ".gserviceaccount.com", "")
...
resource "google_cloud_run_v2_service_iam_member" "cv_invoker" {
  project  = google_cloud_run_v2_service.cv_api.project
  location = google_cloud_run_v2_service.cv_api.location
...
# ==========================================
resource "google_compute_region_network_endpoint_group" "cv_neg" {
  name                  = "neg-cv-${terraform.workspace}"
  network_endpoint_type = "SERVERLESS"
  region                = var.region
  cloud_run {
    service = google_cloud_run_v2_service.cv_api.name
  }
}

resource "google_compute_backend_service" "cv_backend" {
  name                  = "backend-cv-${terraform.workspace}"
  protocol              = "HTTPS"
  load_balancing_scheme = "EXTERNAL_MANAGED"
  security_policy       = google_compute_security_policy.waf.id
  backend {
    group = google_compute_region_network_endpoint_group.cv_neg.id
  }
}

resource "google_compute_region_backend_service" "cv_internal_backend" {
  name                  = "backend-internal-cv-${terraform.workspace}"
  region                = var.region
  protocol              = "HTTP"
...
resource "google_secret_manager_secret_iam_member" "cv_gemini_access" {
