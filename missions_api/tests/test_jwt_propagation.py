...
def test_router_source_extracts_sub_not_hardcoded():
    ...
    token_payload.get("sub") comme user_email, pas une valeur hardcodée.
        """
    source_analysis = (pathlib.Path(__file__).parent.parent / "src" / "missions" / "analysis_router.py").read_text()
    source_crud = (pathlib.Path(__file__).parent.parent / "src" / "missions" / "crud_router.py").read_text()
