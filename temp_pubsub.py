async def handle_pubsub_cv_import(request: Request, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    ...
    async def _run_cv_delete_bg(
        ...
        async with database.SessionLocal() as bg_db:
            try:
                # 1. Archiver le CV
                stmt = select(CVProfile).filter(CVProfile.source_url.like(f"%{bg_google_file_id}%"))
                ...
                if cvs:
                    for cv in cvs:
                        ...
                        user_id = cv.user_id
                    await bg_db.commit()
                    logger.info(f"[PubSub/BG] CV(s) archivé(s) pour le fichier {bg_google_file_id} (user_id={user_id})")
                else:
                    logger.warning(f"[PubSub/BG] Aucun CV trouvé pour le fichier {bg_google_file_id} à archiver.")
                            ...
                            if u_res.status_code == 200:
                                ...
                                if user_data.get("role") != "admin":
                                    ...
                                    if update_res.is_success:
                                        logger.info(f"[PubSub/BG] Utilisateur {user_id} désactivé car non admin.")
                                    else:
                                        logger.error(
                                            f"[PubSub/BG] Échec désactivation user {user_id}: {update_res.text}"
                                        )
                                else:
                                    logger.info(f"[PubSub/BG] Utilisateur {user_id} est admin, on ne le désactive pas.")
                    except Exception as e:
                        logger.error(f"[PubSub/BG] Erreur vérif/désactivation user {user_id}: {e}")
                ...
                try:
                    async with httpx.AsyncClient(timeout=10.0) as patch_client:
                        res = await patch_client.patch(
                            ...
                            json={"status": "DELETED_OK", "error_message": None},
                            headers=bg_headers
                        )
                        if res.is_error:
                            logger.error(f"[PubSub/BG] Échec PATCH DELETED_OK: HTTP {res.status_code}")
                        else:
                            logger.info(f"[PubSub/BG] Notifié drive_api DELETED_OK pour {bg_google_file_id}")
                except Exception as e:
                    logger.warning(f"[PubSub/BG] Impossible de notifier drive_api (DELETED_OK): {e}")
