...
async def get_user_stats(db: AsyncSession = Depends(get_db)):
    ...
    active = (await db.execute(select(func.count()).select_from(User).filter(User.is_active is True))).scalar()
...
async def merge_users(req: MergeRequest, request: Request, db: AsyncSession = Depends(get_db), payload: dict = Depends(verify_jwt)):
    ...
    await delete_cache(f"users:{req.source_id}")
    await delete_cache(f"users:{req.target_id}")
