...
class BulkUsersRequest(BaseModel):
    """Corps de la requête POST /bulk — liste d'IDs utilisateurs validés INT4."""
    user_ids: List[Annotated[int, Field(gt=0, le=2_147_483_647)]] = Field(
        description="Liste d'IDs utilisateurs (INT4). Les IDs hors plage [1, 2147483647] sont rejetés."
    )


@router.post("/bulk", response_model=List[UserResponse])
async def get_users_bulk(body: BulkUsersRequest, db: AsyncSession = Depends(get_db)):
    """
    Retourne les utilisateurs correspondant aux IDs fournis.
    Les IDs hors plage INT4 sont rejetés avec 422 (validation Pydantic).
    """
    if not body.user_ids:
        return []
    users = (await db.execute(select(User).filter(User.id.in_(body.user_ids)))).scalars().all()
...
async def create_user(user: UserCreate, db: AsyncSession = Depends(get_db), payload: dict = Depends(verify_jwt)):
    ...
    try:
        ...
        if existing:
            ...
            existing.full_name = (
                user.full_name
                or (f"{user.first_name} {user.last_name}" if user.first_name and user.last_name else existing.full_name)
            )
