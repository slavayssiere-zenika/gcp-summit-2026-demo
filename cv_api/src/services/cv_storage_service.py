...
class CVStorageService:
    ...
    async def resolve_identity_and_user(
        ...
        llm_first_name = CVStorageService.sanitize_field(
            structured_cv.get("first_name"))
        llm_last_name = CVStorageService.sanitize_field(
            structured_cv.get("last_name"))
        is_anonymous = structured_cv.get("is_anonymous", False)
        trigram = CVStorageService.sanitize_field(structured_cv.get("trigram"))

        if llm_first_name and not CVStorageService.is_valid_name(
                llm_first_name):
            llm_first_name = None
        ...
        if folder_first_name and folder_last_name and llm_first_name and llm_last_name:
            if folder_first_name.lower() != llm_first_name.lower(
            ) or folder_last_name.lower() != llm_last_name.lower():
        ...
        if not CVStorageService.is_valid_email(raw_email):
            if first_name and last_name:
                email = f"{
                    CVStorageService.normalize_str(first_name).replace(
                        ' ',
                        '').replace(
                        '-',
                        '')}.{
                    CVStorageService.normalize_str(last_name).replace(
                        ' ',
                        '').replace(
                        '-',
                        '')}@zenika.com"
                warnings.append(
                    f"Email absent ou invalide dans le CV — email généré : {email}")
            else:
                is_anonymous = True
                trigram = trigram or ''.join(
                    random.choices(string.ascii_uppercase, k=3))
                first_name, last_name = "Anon", trigram
                email = f"anon.{trigram.lower()}@anonymous.zenika.com"
                warnings.append(
                    "Identité introuvable dans le CV — profil anonymisé automatiquement")
        else:
            email = raw_email

        ext_full_norm = f"{
            CVStorageService.normalize_str(first_name)} {
            CVStorageService.normalize_str(last_name)}"
        ...
        async with httpx.AsyncClient(timeout=30.0) as http_client:
            ...
            if not user_id and folder_first_name and folder_last_name:
                ...
                if fn_res.status_code == 200:
                    try:
                        ...
                        logger.error("[cv_storage] Rupture contrat users /search (folder)",
                                     extra={"error": str(ve)})
                        fn_data = None
                    if fn_data:
                        for u in fn_data.items:
                            if CVStorageService.normalize_str(u.first_name) == CVStorageService.normalize_str(
                                    folder_first_name) and CVStorageService.normalize_str(u.last_name) == CVStorageService.normalize_str(folder_last_name):
                                user_id = u.id
                                break
            ...
            if not user_id and email:
                ...
                if search_res.status_code == 200:
                    try:
                        ...
                        logger.error("[cv_storage] Rupture contrat users /search (email)",
                                     extra={"error": str(ve)})
                        search_data = None
                    if search_data:
            ...
            if not user_id and first_name and last_name:
                ...
                if name_res.status_code == 200:
                    try:
                        ...
                        logger.error("[cv_storage] Rupture contrat users /search (name)",
                                     extra={"error": str(ve)})
                        name_data = None
                    if name_data:
                        for u in name_data.items:
                            if CVStorageService.normalize_str(u.first_name) == CVStorageService.normalize_str(
                                    first_name) and CVStorageService.normalize_str(u.last_name) == CVStorageService.normalize_str(last_name):
                                user_id = u.id
                                break
            ...
            if importer_username:
                ...
                if importer_res.status_code == 200:
                    try:
                        ...
                        logger.error("[cv_storage] Rupture contrat users /search (importer)",
                                     extra={"error": str(ve)})
                        importer_data = None
                    if importer_data:
            ...
            if not user_id:
                ...
                if create_res.status_code == 409 or (
                    ...
                    host = email.split(
                        '@')[1] if '@' in email else "zenika.com"
                    prefix = email.split(
                        '@')[0] if '@' in email else "conflict"
                    conflict_email = f"{prefix}.conflict.{
                        random.randint(
                            1000, 9999)}@{host}"
