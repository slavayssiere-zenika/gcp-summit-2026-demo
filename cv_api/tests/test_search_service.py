...
async def test_execute_search_with_skills_provided():
    ...
    # db.execute() is called 3 times in execute_search (feature R6):
    # 1st: missing_count_stmt (.scalar() → 0)
    # 2nd: stmt_filtered (.all() → vector results)
    # 3rd: stmt_unfiltered (.scalar() → total before threshold)
    mock_missing = MagicMock()
    mock_missing.scalar.return_value = 0
    mock_unfiltered = MagicMock()
    mock_unfiltered.scalar.return_value = 3
    db.execute.side_effect = [mock_missing, mock_result, mock_unfiltered]
...
async def test_execute_search_llm_filter_extraction():
    ...
    # db.execute() is called 3 times in execute_search (feature R6):
    # 1st: missing_count_stmt (.scalar() → 5)
    # 2nd: stmt (.all() → vector results, fallback path)
    # 3rd: stmt_unfiltered (.scalar() → total before threshold)
    mock_missing = MagicMock()
    mock_missing.scalar.return_value = 5
    mock_unfiltered = MagicMock()
    mock_unfiltered.scalar.return_value = 10
    db.execute.side_effect = [mock_missing, mock_result, mock_unfiltered]
