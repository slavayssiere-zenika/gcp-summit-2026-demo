...
@pytest.fixture
def mock_redis():
    with patch("src.cvs.task_state.redis.from_url") as mock_from_url:
...
async def test_reanalysis_update_progress(mock_redis):
    state = ReanalysisTaskState()
    mock_redis.get.return_value = json.dumps({"processed_count": 0, "error_count": 0, "mismatch_count": 0, "logs": [], "errors": [], "total_cvs": 10})
